...
import { competitionUuid, useSessionStore } from './store';
  ...
  const connect = useCallback(
    async (pin: string, baseUrl?: string) => {
      ...
      try {
        ...
        const { scope, poule, tournament } = useSessionStore.getState();
        ...
        // que é desta competição** — o resto fica à espera de quem o possa entregar (spec §8).
        void drainQueue(competitionUuid({ poule, tournament }));

        // `scope` decide o ecrã. O árbitro escreve seis dígitos e não sabe — nem tem de saber —
        // que tipo de código lhe deram (contrato §7).
        router.replace(scope === 'tournament' ? '/bracket' : '/poule');
      } catch (failure) {
  ...
  if (failure instanceof ApiError) {
    switch (failure.code) {
      ...
      case 'competition_finished':
        return t('connect.error.finished');
      case 'pin_throttled':
