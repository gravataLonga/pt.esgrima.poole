...
**Versão do contrato: `2.3.1`** · Estado: **feita dos dois lados** · 2026-08-05
...
> novo, nenhum campo alterado, nenhum `type` novo. **A app já o chama** desde a `2.3.1` — e tinha de
> ser ela, porque é ela que sabe que o árbitro saiu do assalto e o servidor não tem como o adivinhar.
> Ver [§11 G](#g-a-230-do-lado-da-app).
>
...
A `2.3.0` foi servida a 2026-08-05 e é a primeira que **não se fecha do lado da plataforma**: o
endpoint existia e ninguém o chamava, porque quem sabe que o árbitro saiu do assalto é a app. O lado
dela ficou feito no mesmo dia, e está em [G](#g-a-230-do-lado-da-app).
...
### G. A `2.3.0` do lado da app

**A `2.2.0` não precisava da app e esta precisava.** É a diferença entre as duas: ali o servidor
...
não tem como distinguir isso de um telemóvel no bolso a meio de um assalto a sério.

| # | O quê | Onde |
|---|---|---|
| G1 | **`DELETE /bouts/{bout}/start` ao sair do assalto sem resultado.** É o `onLeave`, nos dois ramos: o `back()` direto (sem toques marcados) e o `back` do `Alert` de confirmação. *Fire-and-forget*, sem fila — falhar não pode travar quem quer é sair. **Só quando foi este dispositivo a arrancar o cronómetro**: sem `start` não há nada para libertar, e o assalto que está em pista pode ser do árbitro do lado | `src/bout/BoutScreen.tsx`, `app/bout/[id].tsx` |
| G2 | **O mesmo numa sessão de combate**, com `DELETE /elimination/{match}/start`. Não há `back` aqui — não existe ecrã por baixo —, portanto o gatilho é sair da sessão, e o sítio é o `disconnect()`: é por onde passam as duas saídas ("Sair" e o ecrã de resumo), e é onde ainda há token para o pedido levar | `src/session/store.ts`, `app/match/[id].tsx` |
| G3 | **Esquecer o assalto ao largá-lo**, com um `clearStarted(competitionKey)` no mesmo sítio do G1 e no `disconnect()`. **Esquece o assalto e não a pista:** a entrada fica, com o `bout_id` a `null`. São duas perguntas — qual é o meu assalto, e se já arbitrei aqui — e apagar a entrada inteira respondia "não" à segunda, que é a que faz o cartão do topo da lista voltar a propor "Retomar" sobre o assalto de outro | `src/poule/refereeing.ts`, `src/poule/status.ts`, `src/session/store.ts` |
| G4 | **Não chamar o `DELETE` depois de um `score`.** O assalto acabou, não foi abandonado, e o servidor recusa na mesma — mas pedir a libertação de um assalto que se acabou de registar é dizer a coisa errada e desperdiçar um pedido no fim de cada assalto. Vale também para o resultado **em fila**: é o `score` que há de chegar que fecha o assalto | `src/bout/BoutScreen.tsx`, `src/session/store.ts` |

> **O `DELETE /session` continua a ser a rede de segurança, e passou a partir com token.** Ele ia
> depois de a app esquecer o token e saía sem `Authorization`: apanhava `401` e não revogava nada —
> nem, agora, libertaria nada. Numa lista de poule é ele que fecha o que uma app morta em
> *background* deixou aberto, porque aí a app não tem como saber se o assalto que a memória guarda
> já foi pontuado.
...
> comporta como antes. O que **não** se degradava era o contrário — servidor na `2.3.0` e app
> anterior —, e era o estado entre 5 de agosto e a `2.3.1`.
